...
function ImportJSON(url, query, options) {
  var dataReturn=ImportJSONAdvanced(url, query, options, includeXPath_, defaultTransform_);
  return dataReturn;
}


/*====================================================================================================================================*
  ImportHeaderRowJSON by Tao Ren (tour.ren.gz@gmail.com)
  ====================================================================================================================================
  Version:      1.1
  Project Page: https://touren.github.io/2017/08/29/Import_JSON_Data_Returned_From_YouTube_Analytics_API.html
  Copyright:    (c) 2017 by Tao Ren
  License:      GNU General Public License, version 3 (GPL-3.0) 
                http://www.opensource.org/licenses/gpl-3.0.html
 *====================================================================================================================================*/
/**
 * Convert the json retrun from YouTube query report.
 * For example:
 {
...
function ImportHeaderRowJSON(url, options) {
  ...
  var rowCount = rowData.length / columnCount;
  var currentRow=0;
  var table = new Array();
  table[currentRow] = new Array();
  table[currentRow++][0] = "RESULT";
  
  table[currentRow] = new Array();
  for (var i = 0; i < columnCount; i++) {
    table[currentRow][i] = header[i+1][0];
  }
  currentRow ++;
  ...
  for (var r = 0; r < rowCount; r ++) {
    ...
    for (var col = 0; col < columnCount; col ++) {
      var dataType = header[col+1][2];
      if (dataType == "FLOAT") {
        table[currentRow][col] = parseFloat(rowData[r * columnCount + col]);
      } else if (dataType == "INTEGER") {
        table[currentRow][col] = parseInt(rowData[r * columnCount + col]);
      } else {
        table[currentRow][col] = rowData[r * columnCount + col];
      }
    }
    currentRow ++;
  }
